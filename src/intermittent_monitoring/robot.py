...
class Robot():
    def __init__(self, robot_no, motion_config, paths_config):
        ...
        self.battery = self.max_battery #range: [0, 100]
    ...
    def visit_area(self, area):
        ...
        if self.location != 0:
            ...
            if self.battery < self.max_battery:
                request_result = bool(self.charge_battery_request(max_charge=self.max_battery))
    ...
    def raise_fmeasure_action_request(self, area, goal_fmeasure):
        ...
        if result is True:
            #BATTERY DEPLETION
            self.battery -= 0.25
            self.global_batt_consumed += 0.25
    ...
    def charge_battery_request(self, max_charge):
        """
        ...
        goal.max_batt_level = max_charge
        ...
        if result is True:
            self.battery = max_charge #Battery charged to max level
